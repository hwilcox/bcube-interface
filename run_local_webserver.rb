...
class SsiHandler < WEBrick::HTTPServlet::FileHandler
  ...
  def service(req, res)
    super
    return unless @enabled && res.body.path.include?('.html')
    ...
    body.scan(/\<!--#include virtual=".*" --\>/).each do |match|
      ...
      body.insert(
        body.index(match) + match.length,
        ssi_content.gsub(%r{href="/}, "href=\"#{@host}/").gsub(%r{src="/}, "src=\"#{@host}/")
      )
    end
...
def start_webrick(config = {})
  ...
  local_config['webserver']['local']['mount_dirs'].each do |d|
    ...
    server.mount(
      ...
      d['file_path'],
      FancyIndexing: true,
      Host: local_config['webserver']['ssi']['host'],
      Enabled: local_config['webserver']['ssi']['enabled']
    )
  end
  ...
  local_config['webserver']['local'][proxies].each do |search_proxy|
    next unless search_proxy['enabled']

    url = "http://#{search_proxy['remote_host']}:#{search_proxy['port']}#{search_proxy['path']}"
    proxed_paths[url] = search_proxy['server_path']
    server.unmount search_proxy['server_path']
    server.mount_proc search_proxy['server_path'] do |req, res|
      search_url = "#{url}?#{req.query_string}"
      res.status = 200
